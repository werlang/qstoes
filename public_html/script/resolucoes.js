$(document).ready( function(){
	...
	$('#opcoes #aceitar, #opcoes #recusar').click( function() {
		if ($(this).attr('id') == 'aceitar'){
			var action = 'aceita';
			var titulo = "Aceitar resolução";
		}
		else{
			var action = 'recusa';
			var titulo = "Recusar resolução";
		}
		if (!$(this).hasClass('disabled')){
			var posting = $.post( "resolve_resolution.php", {
				action: action,
			...
			posting.done(function( data ) {
				...
				if (data != "feedback"){
					if (data == "aceito")
						show_message("Resolução aceita");
					else if (data == "recusado")
						show_message("Resolução recusada");
					remove_selected(0);
					load_table();
				}
				else{
					var box = "<h2>"+titulo+"</h2><p>Caso deseje, envie uma mensagem para o autor da resolução</p><textarea class='red-input'></textarea><div class='container'><button id='envia' class='button-red can-disable'>Enviar</button><button id='nao' class='button-red can-disable'>Não</button></div>";
...
function load_table(){
	...
	var posting = $.post( "resolution_table.php", {disciplina: id, limit: limit, offset: offset} );
	posting.done(function( data ) {
