<?php
	...
	$sql = "SELECT r.cod as cod, r.envio as envio, r.questao as questao, r.mensagem as mensagem, r.arquivo as arquivo, r.nome, $sql_count FROM resolucoes r INNER JOIN questoes q ON q.cod = r.questao WHERE q.disciplina = $disciplina ORDER BY r.envio LIMIT $limit OFFSET $offset";
	//echo $sql;
	if(!$result = $conn->query($sql)){ die('There was an error running the query [' . $conn->error . ']'); }
